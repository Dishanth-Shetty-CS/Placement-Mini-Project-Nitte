...
<title>Customer Registration | Premium Car Rentals</title>
<link
		...
		<form id="customerForm" novalidate>
			...
			<div class="input-group">
				<span class="input-group-text"><i
					...
					id="Cust_Address" name="Cust_Address" class="form-control"
			...
			<label for="Cust_Confirm_Password" class="form-label">Confirm
				Password</label>
			<div class="input-group">
				<span class="input-group-text"><i class="fas fa-key"></i></span> <input
					type="password" id="Cust_Confirm_Password"
					name="Cust_Confirm_Password" class="form-control"
					placeholder="************">
			</div>
			<div id="cust_confirm_password_error" class="error-message"></div>
...
const fields = {
	...
	password: document.getElementById("Cust_Password"),
	confirm: document.getElementById("Cust_Confirm_Password")
};
const errors = {
	name: document.getElementById("cust_name_error"),
	email: document.getElementById("cust_email_error"),
	phone: document.getElementById("cust_phone_error"),
	address: document.getElementById("cust_address_error"),
	password: document.getElementById("cust_password_error"),
	confirm: document.getElementById("cust_confirm_password_error")
};
...
function validateAllFields() {
	...
	if(nameVal === "") { 
		errors.name.textContent = "Please enter your full name to proceed."; 
		fields.name.parentElement.classList.add("error-border"); 
		valid = false; 
	} else if(!/^[a-zA-Z\s]+$/.test(nameVal)) { 
		errors.name.textContent = "Your name should only contain letters and spaces."; 
		fields.name.parentElement.classList.add("error-border"); 
		valid = false; 
	} else { 
	...
	if(emailVal === "") { 
		errors.email.textContent = "Email address is required for account creation."; 
		fields.email.parentElement.classList.add("error-border"); 
		valid = false; 
	} else if(!emailRegex.test(emailVal)) { 
		errors.email.textContent = "Please provide a valid email address (e.g., user@example.com)."; 
		fields.email.parentElement.classList.add("error-border"); 
		valid = false; 
	} else { 
	...
	if(phoneVal === "") { 
		errors.phone.textContent = "A 10-digit phone number is required for verification."; 
		fields.phone.parentElement.classList.add("error-border"); 
		valid = false; 
	} else if(!/^\d{10}$/.test(phoneVal)) { 
		errors.phone.textContent = "Phone number must consist of exactly 10 numeric digits."; 
		fields.phone.parentElement.classList.add("error-border"); 
		valid = false; 
	} else { 
	...
	if(fields.address.value.trim() === "") { 
		errors.address.textContent = "Please provide your residential address."; 
		fields.address.parentElement.classList.add("error-border"); 
		valid = false; 
	} else { 
		errors.address.textContent = ""; 
		fields.address.parentElement.classList.remove("error-border"); 
	}

	const passVal = fields.password.value;
	const passRegex = /^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{12,}$/;
	if(passVal === "") { 
		errors.password.textContent = "Please create a strong password to secure your account."; 
		...
		errors.password.textContent = "Include at least one number and one special character for better security."; 
		fields.password.parentElement.classList.add("error-border"); 
		valid = false; 
	} else { 
	...
	if(fields.confirm.value === "") { 
		errors.confirm.textContent = "Please confirm your password to avoid entry errors."; 
		fields.confirm.parentElement.classList.add("error-border"); 
		valid = false; 
	} else if(fields.confirm.value !== passVal) { 
		errors.confirm.textContent = "The passwords you entered do not match. Please re-check."; 
		fields.confirm.parentElement.classList.add("error-border"); 
		valid = false; 
	} else { 
...
form.addEventListener("submit", function(e){
	...
	if(validateAllFields()){
		...
		fetch("CustomerServlet",{ method:"POST", headers:{"Content-Type":"application/x-www-form-urlencoded"}, body: params })
		.then(res=>res.json())
		.then(data=>{
			if(data.status==="success"){
				Swal.fire({ title:"Success!", text:"Your account has been created successfully.", icon:"success", confirmButtonColor: "#1e293b" })
				.then(()=>window.location.href="customer-login.jsp?customerId="+data.customerId);
			} else {
				Swal.fire({title:"Registration Failed", text:data.message, icon:"error"});
			}
		})
		.catch(err=>Swal.fire({title:"Error", text:"An unexpected error occurred. Please try again.", icon:"error"}));
	}
});
</script>
</body>
